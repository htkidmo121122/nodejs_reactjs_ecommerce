...
const OrderPage = () => {
  ...
  const diliveryPriceMemo = useMemo(() => {
    if(priceMemo >= 20000000 && priceMemo < 50000000){
      return 10000
  ...
  const itemsDelivery = [
    {
      title: '20.000 VND',
      description: 'Dưới 20.000.000 VND',
    },
    {
      title: '10.000 VND',
      description: 'Từ 20.000.000 VND đến dưới 50.000.000 VND',
    },
    {
      title: 'Free ship',
  ...
  const fetchApplyVoucher = async (context) => {
  
      const res = await axios.post(`http://localhost:3001/api/voucher/apply-voucher`, {
        code: code,
      });
            ...
            <div style={{width: '100%'}}>
              ...
              <WrapperInfo>
                ...
                <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                  ...
                  <span style={{color: '#000', fontSize: '14px', fontWeight: 'bold'}}>{voucher}%</span>
                </div>
                <div style={{display: 'flex-wrap', alignItems: 'center', justifyContent: 'space-between'}}>
                  <span>Mã Voucher</span>
                  <InputComponent size={30}  
                    type="text"
                    value={code}
                    onChange={(e) => setCode(e.target.value)}
                    placeholder={'Nhập mã voucher'} style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>

                  </InputComponent>
                  <ButtonComponent
                      onClick={fetchApplyVoucher}
                      size={40}
                      styleButton={{
